...
class User extends Common_API_Controller {
    ...
    function states_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $country_id = extract_value($data, 'country_id', '');
            $states = $this->common_model->customGet(array('table' => 'states', 'select' => 'id,name', 'where' => array('country_id' => $country_id)));
            if ($states) {
                $return['status'] = 1;
     ...
     */
    function getInstrumentList_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        if ($this->form_validation->run() == FALSE) {
            ...
            $states = $this->common_model->customGet(array('table' => 'services', 'select' => 'id,title as name', 'where' => array('is_active' => 1, 'delete_status' => 0)));
            if ($states) {
                $return['status'] = 1;
                $return['response'] = $states;
                $return['message'] = 'Successfully records found';
     ...
     * Description:   To Get All category
     */
    function add_job_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_category', 'job category', 'trim');
        $this->form_validation->set_rules('received_by', 'received by', 'trim');
        $this->form_validation->set_rules('date_received', 'date received', 'trim');
        $this->form_validation->set_rules('time_received', 'time received', 'trim');
        $this->form_validation->set_rules('date_of_loss', 'date of loss', 'trim');
        if ($this->form_validation->run() == FALSE) {
            $error = $this->form_validation->rest_first_error_string();
            $return['status'] = 0;
            $return['message'] = $error;
            ...
            if (!empty($data['job_id'])) {
                $option = array('table' => 'job', 'select' => 'id', 'where' => array('id' => $data['job_id']), 'single' => true);
                $AvailabeResult = $this->common_model->customGet($option);
                if (!empty($AvailabeResult)) {
                    $option = array(
                        'table' => 'job',
                        'data' => array(
                            ...
                            'date_of_loss' => date('Y-m-d', strtotime($this->input->post('date_of_loss'))),
                            ...
                            'adjuster' => $this->input->post('adjuster'),
                            'adjuster_phone' => $this->input->post('adjuster_phone'),
                            'insurance_email_address' => $this->input->post('insurance_email_address'),
                            'description_of_loss' => $this->input->post('description_of_loss'),
                            'date_time_contracted_insured' => (!empty($this->input->post('date_time_contracted_insured'))) ? date("y-m-d H:i:s", strtotime($this->input->post('date_time_contracted_insured'))) : null,
                            'date_time_appointment_scheduled' => (!empty($this->input->post('date_time_appointment_scheduled'))) ? date("y-m-d H:i:s", strtotime($this->input->post('date_time_appointment_scheduled'))) : null,
                            'how_many_floors_affected' => $this->input->post('how_many_floors_affected'),
                            'flooring_type_affected' => $this->input->post('flooring_type_affected'),
                            'approx_sq_ft_affected' => $this->input->post('approx_sq_ft_affected'),
                            'year_house_was_built' => $this->input->post('year_house_was_built'),
                            'levels' => $this->input->post('levels'),
                            'is_there_standing_water' => $this->input->post('is_there_standing_water'),
                            'is_there_standing_water_source' => $this->input->post('is_there_standing_water_source'),
                            'has_source_been_corrected' => $this->input->post('has_source_been_corrected'),
                            'is_board_up_needed_for_doors_od_windows' => $this->input->post('is_board_up_needed_for_doors_od_windows'),
                            'is_board_up_needed_for_doors_od_windows_how_many' => $this->input->post('is_board_up_needed_for_doors_od_windows_how_many'),
                            'are_there_any_punctures_in_roof' => $this->input->post('are_there_any_punctures_in_roof'),
                            'is_electric_water_available_at_property' => $this->input->post('is_electric_water_available_at_property'),
                            'comments' => $this->input->post('comments'),
                            'lead_source' => $this->input->post('lead_source'),
                            'created_by_id' => $this->user_details->id
                        )
                    );
                    $option['where']['id'] = $data['job_id'];
                    $this->common_model->customUpdate($option);
                    $job_id = $data['job_id'];
                ...
                $job_id = $this->common_model->customInsert($option);
                $message = 'Successfully records added';
            }
            if ($job_id) {
                $return['status'] = 1;
    ...
    function get_job_list_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['search'])) {
                $option['like'] = array('job_category' => $data['search']);
            }
            if (!empty($data['job_id'])) {
                $option['where'] = array('id' => $data['job_id']);
            }
     ...
     * Description:   To Get All category
     */
    function add_authorisation_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cleaners_signature = null;
            $authorized_agent_signature_image = null;
            define('UPLOAD_DIR', 'uploads/users/');
            if (!empty($data['authorized_agent_signature_image'])) {
                $image_parts = explode(";base64,", $data['authorized_agent_signature_image']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $authorized_agent_signature_image = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($authorized_agent_signature_image, $image_base64);
            }

            if (!empty($data['cleaners_signature'])) {
                $image_parts = explode(";base64,", $data['cleaners_signature']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $cleaners_signature = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($cleaners_signature, $image_base64);
            }

            if (!empty($_FILES['cleaners_signature']['name'])) {
                $panCardFile = fileUpload('cleaners_signature', 'users', 'png|jpg|jpeg|gif');
                ...
                $cleaners_signature = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }

            if (!empty($_FILES['authorized_agent_signature_image']['name'])) {
                $panCardFile = fileUpload('authorized_agent_signature_image', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $authorized_agent_signature_image = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }

            $option = array('table' => 'job_authorisation', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_authorisation',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        'property_owner_agent' => $this->input->post('property_owner_agent'),
                        ...
                        'authorized_agent_signature_image' => $authorized_agent_signature_image,
                        'authorized_agent_print_name' => $this->input->post('authorized_agent_print_name'),
                        'authorized_agent_title' => $this->input->post('authorized_agent_title'),
                        'authorized_agent_date' => date('Y-m-d', strtotime($this->input->post('authorized_agent_date'))),
                        //'cleaners_signature' => $cleaners_signature,
                        'cleaners_print_name' => $this->input->post('cleaners_print_name'),
                        'cleaners_title' => $this->input->post('cleaners_title'),
                        'cleaners_date' => date('Y-m-d', strtotime($this->input->post('cleaners_date')))
                    )
                );
                if(!empty($cleaners_signature)){
                    $option['data']['cleaners_signature'] =  $cleaners_signature;
                }
                $option['where']['job_id'] = $data['job_id'];
                $this->common_model->customUpdate($option);
                $job_id = $data['job_id'];
            } else {
                $option = array(
     ...
     * Description:   To Get All category
     */
    function get_authorisation_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_authorisation', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
            if ($list) {
                $return['status'] = 1;
     ...
     * Description:   To Get All category
     */
    function add_antimicrobial_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($_FILES['print_signature']['name'])) {
                $panCardFile = fileUpload('print_signature', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $print_signature = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }

            define('UPLOAD_DIR', 'uploads/users/');
            if (!empty($data['print_signature'])) {
                $image_parts = explode(";base64,", $data['print_signature']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $print_signature = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($print_signature, $image_base64);
            }



            $option = array('table' => 'job_antimicrobial', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_antimicrobial',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        ...
                        'claim' => $this->input->post('claim'),
                        'phone' => $this->input->post('phone'),
                        'source_of_loss' => $this->input->post('source_of_loss'),
                        'category_of_loss' => $this->input->post('category_of_loss'),
                        'antimicrobial_brand' => $this->input->post('antimicrobial_brand'),
                        'application_method' => $this->input->post('application_method'),
                        'areas_of_application' => $this->input->post('areas_of_application'),
                        'materials_applied_to' => $this->input->post('materials_applied_to'),
                        'sds_provided' => $this->input->post('sds_provided'),
                        'chemical_label' => $this->input->post('chemical_label'),
                        'occupants_advised' => $this->input->post('occupants_advised'),
                        'animals_evacuated' => $this->input->post('animals_evacuated'),
                        'permission' => $this->input->post('permission'),
                        'insured_print' => $this->input->post('insured_print'),
                        'print_date' => date('Y-m-d', strtotime($this->input->post('print_date'))),
                        //'print_signature' => $print_signature,
                        'entry_date' => date('Y-m-d H:i:s')
                    )
                );
                if(!empty($print_signature)){
                    $option['data']['print_signature'] =  $print_signature;
                }
                $option['where']['job_id'] = $data['job_id'];
                $this->common_model->customUpdate($option);
                $job_id = $data['job_id'];
     ...
     * Description:   To Get All category
     */
    function get_antimicrobial_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_antimicrobial', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function add_customer_responslbility_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $signature_image_first = null;
            if (!empty($_FILES['signature_image_first']['name'])) {
                $panCardFile = fileUpload('signature_image_first', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $signature_image_first = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }

            $signature_image_second = null;
            if (!empty($_FILES['signature_image_second']['name'])) {
                $panCardFile = fileUpload('signature_image_second', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $signature_image_second = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }


            define('UPLOAD_DIR', 'uploads/users/');
            if (!empty($data['signature_image_first'])) {
                $image_parts = explode(";base64,", $data['signature_image_first']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $signature_image_first = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($signature_image_first, $image_base64);
            }


            if (!empty($data['signature_image_second'])) {
                $image_parts = explode(";base64,", $data['signature_image_second']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $signature_image_second = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($signature_image_second, $image_base64);
            }



            $option = array('table' => 'job_customer_responslbility', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_customer_responslbility',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        'number_of_air_movers' => $this->input->post('number_of_air_movers'),
                        'number_of_axial_fans' => $this->input->post('number_of_axial_fans'),
                        'number_of_dehumidifiers' => $this->input->post('number_of_dehumidifiers'),
                        'specialty_drying_equipment' => $this->input->post('specialty_drying_equipment'),
                        'project_manager' => $this->input->post('project_manager'),
                        'customer_initials' => $this->input->post('customer_initials'),
                        'equipment_placement_date' => date('Y-m-d', strtotime($this->input->post('equipment_placement_date'))),
                        'read_terms_and_conditions' => $this->input->post('read_terms_and_conditions'),
                        'property_address' => $this->input->post('property_address'),
                        'city' => $this->input->post('city'),
                        'state' => $this->input->post('state'),
                        'zip_code' => $this->input->post('zip_code'),
                        'date_first' => date('Y-m-d', strtotime($this->input->post('date_first'))),
                        'date_second' => date('Y-m-d', strtotime($this->input->post('date_second')))
                       // 'signature_image_first' => $signature_image_first,
                       // 'signature_image_second' => $signature_image_second
                    )
                );
                if(!empty($signature_image_first)){
                    $option['data']['signature_image_first'] =  $signature_image_first;
                }
                if(!empty($signature_image_second)){
                    $option['data']['signature_image_second'] =  $signature_image_second;
                }
                $option['where']['job_id'] = $data['job_id'];
                $this->common_model->customUpdate($option);
                $job_id = $data['job_id'];
     ...
     * Description:   To Get All category
     */
    function get_customer_responslbility_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_customer_responslbility', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function add_daily_room_log_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_daily_room_log', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_daily_room_log',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        'job_name' => $this->input->post('job_name'),
                        'address' => $this->input->post('address'),
                        ...
                        'type' => $this->input->post('type'),
                        'auto_fill' => $this->input->post('auto_fill'),
                        'rom' => $this->input->post('rom'),
                        'rolling' => $this->input->post('rolling'),
                        'chamber_rolling' => $this->input->post('chamber_rolling'),
                        'square_foot' => $this->input->post('square_foot'),
                        'fill' => $this->input->post('fill'),
                        'celing' => $this->input->post('celing'),
                        'roller' => $this->input->post('roller'),
                        'cubic_feet' => $this->input->post('cubic_feet'),
                        'auto' => $this->input->post('auto'),
                        'offsets_18' => $this->input->post('offsets_18'),
                        'offsets_rolling' => $this->input->post('offsets_rolling'),
                        'of_air_movers' => $this->input->post('of_air_movers'),
                        'air_rolling' => $this->input->post('air_rolling'),
                        'air_scrubber' => $this->input->post('air_scrubber'),
                        'scrubber_rolling' => $this->input->post('scrubber_rolling'),
                        'etes' => $this->input->post('etes'),
                        'etes_rolling' => $this->input->post('etes_rolling'),
                        'power_box' => $this->input->post('power_box'),
                        'power_rolling' => $this->input->post('power_rolling'),
                        'injection_drying' => $this->input->post('injection_drying'),
                        'injection_rolling' => $this->input->post('injection_rolling'),
                        'holes' => $this->input->post('holes'),
                        'holes_rolling' => $this->input->post('holes_rolling'),
                        'moisture_map' => $this->input->post('moisture_map'),
                        'map_date' => date('Y-m-d', strtotime($this->input->post('map_date'))),
     ...
     * Description:   To Get All category
     */
    function get_daily_room_log_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_daily_room_log', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function add_certificate_of_completion_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($_FILES['owner_signature']['name'])) {
                $panCardFile = fileUpload('owner_signature', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $owner_signature = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }
            ...
            if (!empty($data['owner_signature'])) {
                $image_parts = explode(";base64,", $data['owner_signature']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $owner_signature = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($owner_signature, $image_base64);
            }
            ...
            $option = array('table' => 'job_certificate_of_completion', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_certificate_of_completion',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        ...
                        'date' => date('Y-m-d', strtotime($this->input->post('date'))),
                        'owner_print_date' => date('Y-m-d', strtotime($this->input->post('owner_print_date'))),
                        'cleaners_print_date' => date('Y-m-d', strtotime($this->input->post('cleaners_print_date'))),
                        'year' => $this->input->post('year'),
                        'comments' => $this->input->post('comments'),
                        //'owner_signature' => $owner_signature,
                        'owner_print_name' => $this->input->post('owner_print_name'),
                ...
                if(!empty($owner_signature)){
                    $option['data']['owner_signature'] =  $owner_signature;
                }
                if(!empty($cleaners_signature)){
                    $option['data']['cleaners_signature'] =  $cleaners_signature;
                }
     ...
     * Description:   To Get All category
     */
    function get_certificate_of_completion_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_certificate_of_completion', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function add_required_picture_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($_FILES['technician_signature']['name'])) {
                $panCardFile = fileUpload('technician_signature', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $technician_signature = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }
            
            define('UPLOAD_DIR', 'uploads/users/');
            if (!empty($data['technician_signature'])) {
                $image_parts = explode(";base64,", $data['technician_signature']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $technician_signature = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($technician_signature, $image_base64);
            }

            $option = array('table' => 'job_picture_required', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_picture_required',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        'pic_front_house_overall_condition' => ($this->input->post('pic_front_house_overall_condition')) ? $this->input->post('pic_front_house_overall_condition') : 0,
                        'pic_of_source_loss' => ($this->input->post('pic_of_source_loss')) ? $this->input->post('pic_of_source_loss') : 0,
                        'pic_multi_shot_each_affect_are_capture' => ($this->input->post('pic_multi_shot_each_affect_are_capture')) ? $this->input->post('pic_multi_shot_each_affect_are_capture') : 0,
                        'pic_any_damage_to_contest_or_structure_prior' => ($this->input->post('pic_any_damage_to_contest_or_structure_prior')) ? $this->input->post('pic_any_damage_to_contest_or_structure_prior') : 0,
                        'pic_wall_label_each_room' => ($this->input->post('pic_wall_label_each_room')) ? $this->input->post('pic_wall_label_each_room') : 0,
                        'pic_source_loss' => ($this->input->post('pic_source_loss')) ? $this->input->post('pic_source_loss') : 0,
                        'picture_of' => ($this->input->post('picture_of')) ? $this->input->post('picture_of') : 0,
                        'pic_extraction' => ($this->input->post('pic_extraction')) ? $this->input->post('pic_extraction') : 0,
                        'pic_baseboard_removal' => ($this->input->post('pic_baseboard_removal')) ? $this->input->post('pic_baseboard_removal') : 0,
                        'pic_flooring_being_removed' => ($this->input->post('pic_flooring_being_removed')) ? $this->input->post('pic_flooring_being_removed') : 0,
                        'pic_demo_inprogress' => ($this->input->post('pic_demo_inprogress')) ? $this->input->post('pic_demo_inprogress') : 0,
                        'pic_antimicrobial_being_applied' => ($this->input->post('pic_antimicrobial_being_applied')) ? $this->input->post('pic_antimicrobial_being_applied') : 0,
                        'pic_barries' => ($this->input->post('pic_barries')) ? $this->input->post('pic_barries') : 0,
                        'pic_after_demo_complted' => ($this->input->post('pic_after_demo_complted')) ? $this->input->post('pic_after_demo_complted') : 0,
                        'pic_equipment_place' => ($this->input->post('pic_equipment_place')) ? $this->input->post('pic_equipment_place') : 0,
                        'pic_take_pic_of_the_meter_showing_how' => ($this->input->post('pic_take_pic_of_the_meter_showing_how')) ? $this->input->post('pic_take_pic_of_the_meter_showing_how') : 0,
                        'pic_take_of_photo_area_showing' => ($this->input->post('pic_take_of_photo_area_showing')) ? $this->input->post('pic_take_of_photo_area_showing') : 0,
                        'date' => date('Y-m-d', strtotime($this->input->post('date'))),
                        'technician_signature' => $technician_signature
                    )
                );
                if(!empty($technician_signature)){
                    $option['data']['technician_signature'] =  $technician_signature;
                }
                $option['where']['job_id'] = $data['job_id'];
                $this->common_model->customUpdate($option);
                $job_id = $data['job_id'];
     ...
     * Description:   To Get All category
     */
    function get_required_picture_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_picture_required', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function add_stop_work_order_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $receipt_signature_cleaner = null;
            if (!empty($_FILES['receipt_signature_cleaner']['name'])) {
                $panCardFile = fileUpload('receipt_signature_cleaner', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $receipt_signature_cleaner = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }

            $by_signature_property_owner = null;
            if (!empty($_FILES['by_signature_property_owner']['name'])) {
                $panCardFile = fileUpload('by_signature_property_owner', 'users', 'png|jpg|jpeg|gif');
                if (isset($panCardFile['error']) && !empty($panCardFile['error'])) {
                    $return['message'] = strip_tags($panCardFile['error']);
                    $return['status'] = 0;
                    $this->response($return);
                ...
                $by_signature_property_owner = 'uploads/users/' . $panCardFile['upload_data']['file_name'];
            }
            
            define('UPLOAD_DIR', 'uploads/users/');
            if (!empty($data['receipt_signature_cleaner'])) {
                $image_parts = explode(";base64,", $data['receipt_signature_cleaner']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $receipt_signature_cleaner = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($receipt_signature_cleaner, $image_base64);
            }
            
            if (!empty($data['by_signature_property_owner'])) {
                $image_parts = explode(";base64,", $data['by_signature_property_owner']);
                $image_type_aux = explode("image/", $image_parts[0]);
                $image_type = $image_type_aux[1];
                $image_base64 = base64_decode($image_parts[1]);
                $by_signature_property_owner = UPLOAD_DIR . uniqid() . '.png';
                file_put_contents($by_signature_property_owner, $image_base64);
            }
            
            

            $option = array('table' => 'job_stop_work_order', 'select' => 'job_id', 'where' => array('job_id' => $data['job_id']), 'single' => true);
            $AvailabeResult = $this->common_model->customGet($option);
            if (!empty($AvailabeResult)) {
                $option = array(
                    'table' => 'job_stop_work_order',
                    'data' => array(
                        'job_id' => $this->input->post('job_id'),
                        'property_owner' => $this->input->post('property_owner'),
                        'property_address' => $this->input->post('property_address'),
                        'structure_or_building_materials_are_not_com' => $this->input->post('structure_or_building_materials_are_not_com'),
                        'cleaning_of_following_are_structure' => $this->input->post('cleaning_of_following_are_structure'),
                        'other_circumstances' => $this->input->post('other_circumstances'),
                        'issued_on_this' => $this->input->post('issued_on_this'),
                        'day_of' => $this->input->post('day_of'),
                        'year' => $this->input->post('year'),
                        //'receipt_signature_cleaner' => $receipt_signature_cleaner,
                       // 'by_signature_property_owner' => $by_signature_property_owner,
                        'entry_date' => date('Y-m-d H:i:s')
                    )
                );
                if(!empty($receipt_signature_cleaner)){
                    $option['data']['receipt_signature_cleaner'] =  $receipt_signature_cleaner;
                }
                if(!empty($by_signature_property_owner)){
                    $option['data']['by_signature_property_owner'] =  $by_signature_property_owner;
                }
                $option['where']['job_id'] = $data['job_id'];
                $this->common_model->customUpdate($option);
                $job_id = $data['job_id'];
     ...
     * Description:   To Get All category
     */
    function get_stop_work_order_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_stop_work_order', 'select' => '*', 'order' => array('id' => 'DESC'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
     ...
     * Description:   To Get All category
     */
    function get_open_job_list_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job', 'select' => '*', 'order' => array('id' => 'DESC'), 'where' => array('status' => 'Open'));
            if (!empty($data['job_id'])) {
                $option['where'] = array('job_id' => $data['job_id']);
            }
            $list = $this->common_model->customGet($option);
            ...
            foreach($list as $rows){
                $temp['id'] = $rows->id;
                $temp['job_category'] = $rows->job_category;
                $temp['received_by'] = $rows->received_by;
                $temp['date_received'] = $rows->date_received;
                $temp['time_received'] = $rows->time_received;
                $temp['insured_name'] = $rows->insured_name;
                $temp['type'] = $rows->type;
                $temp['job_name'] = $rows->job_name;
                $date1=date_create($rows->date_received);
                $date2=date_create(date('Y-m-d'));
                $diff=date_diff($date1,$date2);
                $temp['open_day'] = $diff->days;
                $Jobs[] = $temp;
            }
            if ($list) {
                $return['status'] = 1;
     ...
     * Description:   To Get All category
     */
    function get_open_job_form_details_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        if ($this->form_validation->run() == FALSE) {
            ...
            $forms = array(
                'job' => 'Call Log',
                'job_authorisation' => 'Authorisation',
                'job_certificate_of_completion' => 'Certificate Of Completion',
                'job_antimicrobial' => 'Antimicrobial',
                'job_customer_responslbility' => 'Customer Responslbility',
                'job_daily_room_log' => 'Daily Room Log',
                'job_picture_required' => 'Picture Required',
                'job_stop_work_order' => 'Stop Work Order'
            );
            foreach ($forms as $key => $rows) {
                ...
                $option = array('table' => $key, 'select' => '*', 'order' => array('id' => 'DESC'), 'where' => $where);
                $temp['name'] = $rows;
                $temp['is_submit'] = $this->common_model->customCount($option);
                $list[] = $temp;
            }
            if ($list) {
                $return['status'] = 1;
            ...
            $return['message'] = $error;
                ...
                $option = array(
                    ...
                    'data' => array(
                        ...
                        'claim' => $this->input->post('claim'),
                        'claim_time_per_room' => $this->input->post('claim_time_per_room'),
                        'contents_blocked' => $this->input->post('contents_blocked'),
                        'contents_time_per_room' => $this->input->post('contents_time_per_room'),
                        'contents_boxed' => $this->input->post('contents_boxed'),
                        'quantity' => $this->input->post('quantity'),
                        'wand_extraction' => $this->input->post('wand_extraction'),
                        'wand_sqr_feet' => $this->input->post('wand_sqr_feet'),
                        'wand_reg' => $this->input->post('wand_reg'),
                        'wand_hvy' => $this->input->post('wand_hvy'),
                        'flooring_removed' => $this->input->post('flooring_removed'),
                        'flooring_type' => $this->input->post('flooring_type'),
                        'flooring_sqr_feet' => $this->input->post('flooring_sqr_feet'),
                        'underlayment_removed' => $this->input->post('underlayment_removed'),
                        'underlayment_type' => $this->input->post('underlayment_type'),
                        'underlayment_sqr_feet' => $this->input->post('underlayment_sqr_feet'),
                        'baseboard_removed' => $this->input->post('baseboard_removed'),
                        'baseboard_ln_ft' => $this->input->post('baseboard_ln_ft'),
                        'doortrim_removed' => $this->input->post('doortrim_removed'),
                        'doortrim_ln_ft' => $this->input->post('doortrim_ln_ft'),
                        'quarter_round_removed' => $this->input->post('quarter_round_removed'),
                        'quarter_round_ln_ft_1' => $this->input->post('quarter_round_ln_ft_1'),
                        'quarter_round_ln_ft_2' => $this->input->post('quarter_round_ln_ft_2'),
                        'door_casing_removed' => $this->input->post('door_casing_removed'),
                        'door_casing_ln_ft_1' => $this->input->post('door_casing_ln_ft_1'),
                        'door_casing_ln_ft_2' => $this->input->post('door_casing_ln_ft_2'),
                        'drywall' => $this->input->post('drywall'),
                        'drywall_sqr_ft' => $this->input->post('drywall_sqr_ft'),
                        'drywall_lf' => $this->input->post('drywall_lf'),
                        'inches_high' => $this->input->post('inches_high'),
                        'plaster' => $this->input->post('plaster'),
                        'plaster_sqr_ft' => $this->input->post('plaster_sqr_ft'),
                        'insulation' => $this->input->post('insulation'),
                        'blown_in' => $this->input->post('blown_in'),
                        'batt' => $this->input->post('batt'),
                        'toe_kicks_removed' => $this->input->post('toe_kicks_removed'),
                        'toe_kicks_ln_ft' => $this->input->post('toe_kicks_ln_ft'),
                        'toe_kicks_drilled' => $this->input->post('toe_kicks_drilled'),
                        'toe_kicks_drilled_ln_ft' => $this->input->post('toe_kicks_drilled_ln_ft'),
                        'full_height_cabs_removed' => $this->input->post('full_height_cabs_removed'),
                        'full_height_cabs_ln_ft' => $this->input->post('full_height_cabs_ln_ft'),
                        'upper_caps_removed' => $this->input->post('upper_caps_removed'),
                        'upper_caps_ln_ft' => $this->input->post('upper_caps_ln_ft'),
                        'lower_caps_removed' => $this->input->post('lower_caps_removed'),
                        'lower_caps_ln_ft' => $this->input->post('lower_caps_ln_ft'),
                        'faces_datached' => $this->input->post('faces_datached'),
                        'faces_datached_sqr_ft' => $this->input->post('faces_datached_sqr_ft'),
                        'counter_top_detached' => $this->input->post('counter_top_detached'),
                        'counter_top_detached_ln_ft' => $this->input->post('counter_top_detached_ln_ft'),
                        'counter_top_detached_type' => $this->input->post('counter_top_detached_type'),
                        'range_circle' => $this->input->post('range_circle'),
                        'electric' => $this->input->post('electric'),
                        'gas' => $this->input->post('gas'),
                        'microwave' => $this->input->post('microwave'),
                        'detached' => $this->input->post('detached'),
                        'dish_washer' => $this->input->post('dish_washer'),
                        'dish_washer_electric' => $this->input->post('dish_washer_electric'),
                        'dish_washer_gas' => $this->input->post('dish_washer_gas'),
                        'sink' => $this->input->post('sink'),
                        'sink_type' => $this->input->post('sink_type'),
                        'sink_other' => $this->input->post('sink_other'),
                        'garbage_disposal' => $this->input->post('garbage_disposal'),
                        'notes' => $this->input->post('notes'),
                        'ro_system' => $this->input->post('ro_system'),
                        'refrigerator' => $this->input->post('refrigerator'),
                        'regular_built_in' => $this->input->post('regular_built_in'),
                        'doors' => $this->input->post('doors'),
                        'doors_type' => $this->input->post('doors_type'),
                        'doors_quantity' => $this->input->post('doors_quantity'),
                        'other' => $this->input->post('other'),
                        'other_notes' => $this->input->post('other_notes'),
                        'critical_barrier' => $this->input->post('critical_barrier'),
                        'critical_sqr_ft' => $this->input->post('critical_sqr_ft'),
                        'tension_posts' => $this->input->post('tension_posts'),
                        'tension_how_many' => $this->input->post('tension_how_many'),
                        'tension_total_days' => $this->input->post('tension_total_days'),
                        'cover_plastic' => $this->input->post('cover_plastic'),
                        'cover_sqr_ft' => $this->input->post('cover_sqr_ft'),
                        'antimicrobial' => $this->input->post('antimicrobial'),
                        'anticibritial_sqr_ft' => $this->input->post('anticibritial_sqr_ft'),
                        'techinican' => $this->input->post('techinican'),
    ...
    function update_room_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $return['message'] = $error;
                ...
                $option['where']['id'] = $this->input->post('room_id');
                $job_id = $this->common_model->customUpdate($option);
    ...
    function get_room_list_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array('table' => 'job_rooms', 'select' => '*', 'order' => array('id' => 'DESC'));
            $option['where'] = array('job_id' => $data['job_id']);
            $list = $this->common_model->customGet($option);
            if ($list) {
                foreach($list as $key=>$rows){
                    $option = array('table' => 'equipment_tools E', 
                        'select' => 'E.*,S.title',
                        'join'=> array('services S' => 'S.id=E.instrument_id'),
                        'order' => array('E.id' => 'DESC')
                        );
                        $option['where'] = array('E.job_id' => $data['job_id']);
                        $option['where'] = array('E.room_id' => $rows->id);
                        $l = $this->common_model->customGet($option);
                        $list[$key]->equipment_list = $l;
                }
                $return['status'] = 1;
                $return['response'] = $list;
                $return['message'] = 'Successfully records found';
     ...
     * Description:   To Get All category
     */
    function add_room_equipment_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        $this->form_validation->set_rules('room_id', 'room Id', 'trim|required|numeric');
        $this->form_validation->set_rules('instrument_id', 'instrument Id', 'trim|required|numeric');
     ...
     * Description:   To Get All category
     */
    function update_room_equipment_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $return['message'] = $error;
                ...
                $option['where']['job_id'] = $this->input->post('job_id');
                $option['where']['room_id'] = $this->input->post('room_id');
                $option['where']['instrument_id'] = $this->input->post('instrument_id');
                $job_id = $this->common_model->customUpdate($option);
            if ($job_id) {
                $return['status'] = 1;
                $return['message'] = 'Successfully records update';
            } else {
                $return['status'] = 0;
                $return['message'] = 'Records not added';
            }
    ...
    function update_room_equipment_all_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option['where'] = array('E.job_id' => $data['job_id']);
            $option['where'] = array('E.room_id' => $data['room_id']);
            $option['where'] = array('E.instrument_id' => $data['instrument_id']);
            $list = $this->common_model->customGet($option);
            if(empty($list)){
                $option = array(
                    'table' => 'equipment_tools',
     ...
     * Description:   To Get All category
     */
    function get_room_equipment_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('job_id', 'job Id', 'trim|required|numeric');
    ...
    function patient_list_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array(
                'table' => 'patient P',
                'select' => 'P.id as pid, P.patient_id as patient_id,P.name as patient_name,P.address,P.symptom_onset,P.md_stayward_consult,P.md_stayward_response,P.created_date,'
                ...
                'join' => array(array('care_unit CI', 'CI.id=P.care_unit_id', 'inner'),
                    ...
                    array('initial_dx IDX', 'IDX.id=PC.initial_dx', 'inner'),
    ...
    function patient_details_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array(
                'table' => 'patient P',
                'select' => 'P.md_patient_status,P.infection_surveillance_checklist,P.total_days_of_patient_stay,P.patient_id as patient_id,P.name as patient_name,P.address,P.symptom_onset,P.md_stayward_consult,P.md_stayward_response,P.created_date,'
                . 'P.care_unit_id,CI.name as care_unit_name,P.doctor_id,DOC.name as doctor_name,P.md_steward_id,U.first_name as md_steward,'
                . 'PC.initial_rx,IRX.name as initial_rx_name,PC.initial_dx,IDX.name as initial_dx_name,PC.initial_dot,'
                . 'PC.new_initial_rx,IRX2.name as new_initial_rx_name,PC.new_initial_dx,IDX2.name as new_initial_dx_name,PC.new_initial_dot,PC.comment',
                'join' => array(array('care_unit CI', 'CI.id=P.care_unit_id', 'inner'),
