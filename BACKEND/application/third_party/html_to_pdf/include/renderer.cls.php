  ...
  function render(Frame $frame, $stacking = false) {
    ...
    if ( $render_self ) {
      ...
      if ( $style->overflow === "hidden" ) {
		...
		// get border radii
		$style = $frame->get_style();
		$rTL = $style->__get("border_top_left_radius");
		$rTR = $style->__get("border_top_right_radius");
		$rBL = $style->__get("border_bottom_left_radius");
		$rBR = $style->__get("border_bottom_right_radius");
		
		// reduce radii if they're larger than half the sides
		$maxRadius = min($w/2, $h/2);
		$rTL = min($rTL, $maxRadius);
		$rTR = min($rTR, $maxRadius);
		$rBL = min($rBL, $maxRadius);
		$rBR = min($rBR, $maxRadius);
		
		$rMax = max($rTL, $rTR, $rBL, $rBR);
		
		if ($rMax > 0)
		{
        	$this->_canvas->clipping_roundrectangle($x, $y, $w, $h, $rTL, $rTR, $rBR, $rBL);
		}
		else
		{
