...
class Block_Renderer extends Abstract_Renderer {
  ...
  protected function _render_border(Frame_Decorator $frame, $corner_style = "bevel") {
    ...
    // find the radius
    $radius["top-left"] = $style->__get("border_top_left_radius");
    $radius["top-right"] = $style->__get("border_top_right_radius");
    $radius["bottom-left"] = $style->__get("border_bottom_left_radius");
    $radius["bottom-right"] = $style->__get("border_bottom_right_radius");

    // reduce radii if they're larger than half the sides + half the border widths
	$maxWidth = max($bp["top"]["width"], $bp["right"]["width"], $bp["bottom"]["width"], $bp["left"]["width"]);
    $maxRadius = min($bbox["w"]/2, $bbox["h"]/2) - $maxWidth/2;
    $radius["top-left"] = min($radius["top-left"], $maxRadius);
    $radius["top-right"] = min($radius["top-right"], $maxRadius);
    $radius["bottom-left"] = min($radius["bottom-left"], $maxRadius);
    $radius["bottom-right"] = min($radius["bottom-right"], $maxRadius);
