...
class CPDF_Adapter implements Canvas {
  ...
  function ellipse($x1, $y1, $r1, $r2, $angle, $astart, $aend, $color, $width, $style = array()) {
    $this->_set_stroke_color($color);
    //if (isset($width) )
    //  $this->_set_line_style($width, "round", "round", $style);

  	$this->_pdf->ellipse($x1, $this->y($y1), $r1, $r2, $angle, 8, $astart, $aend, false, false, true, false);
  }
  ...
  function clipping_roundrectangle($x1, $y1, $w, $h, $rTL, $rTR, $rBR, $rBL) {
	...
	// the eclipse function goes anti-clockwise, so we'll have to go anti-clockise too
		
	// start: top edge, left end
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= sprintf("\n%.3F %.3F m ", $x1 + $rTL, $this->y($y1));
	// curve: top-left corner
	$this->_pdf->ellipse($x1 + $rTL, $this->y($y1 + $rTL), $rTL, 0, 0, 8, 90, 180, false, false, false, true);
	// line:  left edge, bottom end
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= sprintf("\n%.3F %.3F l ", $x1, $this->y($y1 + $h - $rBL));
	// curve: bottom-left corner
	$this->_pdf->ellipse($x1 + $rBL, $this->y($y1 + $h - $rBL), $rBL, 0, 0, 8, 180, 270, false, false, false, true);
	// line: bottom edge, right end
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= sprintf("\n%.3F %.3F l ", $x1 + $w - $rBR, $this->y($y1 + $h));
	// curve: bottom-right corner
	$this->_pdf->ellipse($x1 + $w - $rBR, $this->y($y1 + $h - $rBR), $rBR, 0, 0, 8, 270, 360, false, false, false, true);
	// line: right edge, top end
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= sprintf("\n%.3F %.3F l ", $x1 + $w, $this->y($y1 + $rTR));
	// curve: top-right corner
	$this->_pdf->ellipse($x1 + $w - $rTR, $this->y($y1 + $rTR), $rTR, 0, 0, 8, 0, 90, false, false, false, true);
	// line: top edge, left end
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= sprintf("\n%.3F %.3F l ", $x1 + $rTL, $this->y($y1));

	// Close & clip
	$this->_pdf->objects[$this->_pdf->currentContents]['c'] .= " W n";
  }
